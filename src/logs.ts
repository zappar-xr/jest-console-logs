 ...
 */
export const expectSequential = (opts: Opts1): Promise<void> =>
 ...
 * @returns - A promise which resolves if expectations were met.
 */
export const expectLogs = (opts: Opts2): Promise<void> =>
  new Promise((resolve, reject) => {
...
export const waitForLogs = async (log: string, page: Page, timeoutMs: number): Promise<void> =>
  new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      page.off("console", compare);
      reject(new Error("Timeout waiting for console logs"));
